...
def get_array_until(numbers_value: int) -> List:
    """Return the array with values from 0 until numbers_value"""

    return [x for x in range(numbers_value)]


def get_numbers_which_div_to(divider: int, numbers_value: Any) -> List:
    """Print the array with numbers with share to number from numbers_range"""

    answer = []
    if isinstance(numbers_value, int):
        range_numbers = get_array_until(numbers_value)
...
if __name__ == '__main__':
    t1 = Thread(target=get_numbers_which_div_to, args=([3, 123]))
    t1.start()
